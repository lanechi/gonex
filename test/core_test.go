...
func TestBindRejectsInvalidAndDuplicateRoutes(t *testing.T) {
	if err := ghttp.NewServer().Bind(&invalidController{}); err == nil || !strings.Contains(err.Error(), "Invalid") {
...
func TestRouteRegistrySnapshotAndOpenAPIPath(t *testing.T) {
	...
	if err := registry.Register(router.Definition{Method: http.MethodGet, Path: "/users/:id"}); err != nil {
		t.Fatal(err)
	}
