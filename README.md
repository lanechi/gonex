...
- Memory、签名 Cookie、Redis Session；
...
server := ghttp.NewServer(
	ghttp.WithOpenAPI(true),
	ghttp.WithOpenAPIPath("/api/openapi.json"),
	ghttp.WithSwaggerPath("/api/docs"),
)
```

`WithOpenAPI(false)` 和 `EnableOpenAPI(false)` 会同时关闭 JSON 与 UI。Swagger 模板可通过
...
`session.ContextStorage`（或等价的 `ghttp.ContextSessionStorage`）接收请求 Context；内置 Redis
后端已实现该扩展。

签名 Cookie Session 每次修改都会轮换 Token，并在当前进程内撤销旧 Token；Logout 撤销整个会话族，
防止此前签发的 Token 被重放。撤销表是进程内状态，多副本或跨重启的强制注销应使用 Redis 等服务端
存储。
...
路由、OpenAPI、Logger、Session、模板和生命周期状态按 Server 隔离。Gin mode 是进程级设置，
